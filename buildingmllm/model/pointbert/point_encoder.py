...
class PointTransformer(nn.Module):
    ...
    def forward(self, pts):
        ...
        neighborhood, center, neighborhood_xyz, neighborhood_rgb, center_RGB = self.group_divider(pts)
        #print(neighborhood.shape) #torch.Size([1, 512, 32, 6])
        #print(center.shape) #torch.Size([1, 512, 3])
        #print(neighborhood_xyz.shape) #torch.Size([1, 512, 32, 3])
        #print(neighborhood_rgb.shape) #torch.Size([1, 512, 32, 3])
        #print(center_RGB.shape) #torch.Size([1, 512, 3])
        # encoder the input cloud blocks
        group_input_tokens,knn_x = self.encoder(neighborhood)  # B G N
        #print(group_input_tokens.shape) # torch.Size([1, 512, 256])
        #print(knn_x.shape) #torch.Size([1, 512, 256, 32])
        group_input_tokens = self.reduce_dim(group_input_tokens)
        #print(group_input_tokens.shape) #torch.Size([1, 512, 384])
        # prepare cls
        cls_tokens = self.cls_token.expand(group_input_tokens.size(0), -1, -1)
        #print(cls_tokens.shape) #torch.Size([1, 1, 384])
        cls_pos = self.cls_pos.expand(group_input_tokens.size(0), -1, -1)
        #print(cls_pos.shape) # torch.Size([1, 1, 384])
        # add pos embedding
        pos = self.pos_embed(center)
        #print(pos.shape) #torch.Size([1, 512, 384])
        # final input
        x = torch.cat((cls_tokens, group_input_tokens), dim=1)
        #print(x.shape) # torch.Size([1, 513, 384])
        pos = torch.cat((cls_pos, pos), dim=1)
        #print(pos.shape) #torch.Size([1, 513, 384])
        # transformer
        x = self.blocks(x, pos)
        #print(x.shape) #torch.Size([1, 513, 384])
        ...
        xyz = center #✅ xyz shape: torch.Size([1, 512, 3])
        lc_x = group_input_tokens #✅ lc_x shape: torch.Size([1, 512, 384])
        rgb = center_RGB #✅ rgb shape: torch.Size([1, 512, 3])
        knn_xyz = neighborhood_xyz #✅ knn_xyz shape: torch.Size([1, 512, 32, 3])
        knn_x = knn_x.permute(0, 1, 3, 2) # torch.Size([1, 512, 32, 256])  #需要把最后一维度256 变成384
        #print(knn_x.shape)
        #knn_x = lc_x.unsqueeze(2).expand(-1, -1, 32, -1)#✅ knn_x shape: torch.Size([1, 512, 32, 384])
        knn_rgb = neighborhood_rgb #✅ knn_rgb shape: torch.Size([1, 512, 32, 3])
        #print(x.shape) #torch.Size([1, 513, 384])
        ...
        if not self.use_max_pool:
            return x, xyz, lc_x, rgb, knn_xyz, knn_x, knn_rgb
