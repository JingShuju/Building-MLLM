...
class Group(nn.Module):
    ...
    def forward(self, xyz):
        ...
        center, fps_idx = misc.fps(xyz, self.num_group, return_index=True)
        center_RGB = torch.gather(rgb, 1, fps_idx.unsqueeze(-1).expand(-1, -1, rgb.shape[-1]))
        """print(center.shape)
        print(center_RGB.shape)
        from pointllm.model.pointbert.misc import index_points  # ✅ 如果 index_points 在其它文件中，确保你正确 import

        check_xyz = torch.allclose(center, index_points(xyz, fps_idx), atol=1e-6)
        print("✅ Center 坐标匹配:", check_xyz)

        check_rgb = torch.allclose(center_RGB, index_points(rgb, fps_idx), atol=1e-6)
        print("✅ Center RGB 匹配:", check_rgb)

        # 如果你还想打印具体值（比如第一个样本的第一个点）：
        print("center[0, 0] =", center[0, 0])
        print("xyz[0, fps_idx[0, 0]] =", xyz[0, fps_idx[0, 0]])
        print("center_RGB[0, 0] =", center_RGB[0, 0])
        print("rgb[0, fps_idx[0, 0]] =", rgb[0, fps_idx[0, 0]])"""
        ...
        return neighborhood, center, neighborhood_xyz, neighborhood_rgb, center_RGB
...
class Encoder(nn.Module):
    ...
    def forward(self, point_groups):
        ...
        #print(feature.shape) #torch.Size([512, 256, 32])
        feature_global = torch.max(feature, dim=2, keepdim=False)[0]  # BG 1024
        #print(feature_global.shape) #torch.Size([512, 256])
        #feature_global = feature_global.reshape(bs, g, self.encoder_channel)
        #print(feature_global.shape) #torch.Size([1, 512, 256])
        #feature = feature.reshape(bs, g, self.encoder_channel, n)
        #print(feature.shape) #torch.Size([1, 512, 256, 32])
        return feature_global.reshape(bs, g, self.encoder_channel), feature.reshape(bs, g, self.encoder_channel, n)
