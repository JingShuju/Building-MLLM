...
gpt4_open_free_from_cls_prompt = """Analyze two sentences and determine if they’re referring to the same general object or concept,
focusing on the type of object only, not attributes. Respond with "T" if they refer to the same
thing and "F" if not. Also, provide a brief rationale (no more than 30 words) for your judgment.

Input: 1. A lateral pipe junction fitting. 2. You are looking at a lateral pipe junction fitting.
Output: T#Both refer to the same type of pipe fitting.
...
gpt4_object_captioning_prompt = """Evaluate a model-generated caption against a human-generated caption (ground truth) for a
3D model. First, extract the key aspects mentioned in the human caption. Possible aspects
include, but are not limited to: category, shape, structural features, function/usage, installation/connection, distinguishing attributes.

Be generous when judging similarity. Score from 0 to 100, where each aspect contributes equally
to the score. Consider similar concepts for partial score. Provide your score (0-100) and a short
justification (less than 30 words) in the format of "score#reason".

Example:
Human: The object is a railing with vertical balusters and horizontal top and bottom rails. It
features a linear, elongated structure with evenly spaced vertical elements. The connection forms
include welded joints, providing stability and rigidity. Its distinguishing feature is the uniform
spacing of balusters, ensuring safety and aesthetic appeal.
Model: The object is a railing with a sturdy rectangular top and side panels. It has a linear
geometry and evenly spaced vertical bars, creating a balanced structure. The railing offers safety
and support, typically found in staircases or boundaries.
Output: 90#mentioned railing category, structural features, function, distinguishing attributes;
geometry similar; connection missing.

Now score the following:
Human: {ground_truth}
Model: {model_output}
Output: """
#Evaluate a model-generated caption against a human-generated caption (ground truth) for a 3D model.             #把模型生成的字幕和人工生成的字幕进行对比
# #Identify the aspects mentioned in the human caption                                                           #先把人工 caption 里的 关键方面（aspects） 抽取出来
# #and calculate the percentage of these aspects correctly mentioned or partially matched in the model caption.    #看看模型caption 里有没有提到这些方面： 正确算全对 相似算部分匹配 完全没提到算是没匹配 然后计算匹配比例
# #Score from 0 to 100, where each aspect contributes equally to the score.                                      #每个方面占据比重相同，
# #Consider similar concepts for partial score.    #如果模型没有完全说对 但是说的接近类似的概念 就给部分分
