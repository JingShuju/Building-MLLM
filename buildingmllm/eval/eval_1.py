...
def init_model(args):
   ...
   # 1. 加载 safetensors 权重文件
    safetensor_path = "outputs/JING_train_stage1/JING_stage1_20250815_1111/model-00006-of-00006.safetensors"
    state_dict = load_file(safetensor_path)
    # 2. 修复 gamma 参数名
    if 'model.point_tune.gamma' in state_dict:
        state_dict['point_tune.gamma'] = state_dict.pop('model.point_tune.gamma')"""
    ...
    # 2. 打印 gamma 参数
    print("\n🎯 以下是权重文件中所有包含 'gamma' 的 key：")
    for k in state_dict:
        if "gamma" in k:
            print(k, state_dict[k].shape)"""

    model = PointLLMLlamaForCausalLM.from_pretrained(model_name, low_cpu_mem_usage=False, use_cache=True, torch_dtype=torch.float32).cuda() #torch_dtype=torch.bfloat16 #torch_dtype=torch.float32
    """print("📌 模型结构中所有包含 gamma 的参数名：")
    ...
    for name, param in model.named_parameters():
        ...
        values = param.flatten()[:min(1, param.numel())].tolist()  # 取前 5 个数
        print(f"{name}: {shape}, values: {values}")"""
...
def main(args):
    ...
    if not os.path.exists(args.output_file_path):
        ...
        #--conversation_types "1_General_Visual_Recognition" "2_Knowledge_Capability" "3_Commonsense_Reasoning" "4_Advanced_Engineering_Reasoning" "5_Functional_Comparison" "6_Constraint_Reasoning" "7_Spatial_Relationship" "8_Embodied_Interaction" "9_1-8" "10_2-8"  \
        ...
        """task_type = "1_General_Visual_Recognition"
        annos = {
            anno["object_id"]: anno["conversations"][1]["value"]
            for anno in annos
            if anno["conversation_type"] == task_type
        }"""
...
if __name__ == "__main__":
    ...
    parser.add_argument("--anno_path", type=str, default="", required=False)
