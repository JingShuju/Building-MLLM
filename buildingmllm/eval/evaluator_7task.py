...
jing_object_7_task_prompt= """Here is {task_type} task.
Evaluate a model-generated answer against a human-generated answer (ground truth) for a 3D model.
Identify the aspects mentioned in the human answer, and calculate the percentage of correctly mentioned or matched in the model answer. 
Please give a score from 0 to 100 based on how well the model answer matches the ground truth in the format of 'score#reason.

Example:
Question: What is the function of this object?
Human: To change airflow direction in HVAC systems.
Model: To redirect airflow in duct systems.
Output: 100# mention change airflow direction.
...
Model: {model_output}
Output: """
#把模型生成的回答和人工回答进行对比，目前的任务类型是 {task_type}，请根据任务类型，问题和人工回答，看看模型回答匹配度，Please give a score from 0 to 100 based on how well the model answer matches the ground truth.


chatgpt_object_captioning_prompt = jing_object_7_task_prompt
...
class OpenAIObjectCaptioningEvaluator(OpenAIOpenFreeFormClsEvaluator):
    def __init__(self, inputs, output_dir, output_file, model_type="gpt-4-0613"):
        ...
        self.task_type = inputs.get("task_type", "General")
    ...
    def evaluate_result(self, result):
        ...
        ground_truth = result['ground_truth']
        model_output = result['model_output']
        question = result.get("question", "N/A")  # 默认值防止缺失
        #messages = [{"role": "user", "content": self.gpt_prompt.format(ground_truth=ground_truth, model_output=model_output)}]
        #task_type = result.get("task_type", "Captioning")  # 默认值
        ...
        return object_id, question, model_output, ground_truth, gpt_score, reason, prompt_tokens, completion_tokens
    ...
    def evaluate(self):
        ...
        try:
            for result in tqdm(results):  
                ...
                object_id, question, model_output, ground_truth, gpt_score, reason, prompt_tokens, completion_tokens = self.evaluate_result(result)
    ...
    def parallel_evaluate(self, num_workers=20):
        ...
        try:
            with Pool(num_workers) as pool:
                with tqdm(total=len(results)) as pbar:  # create a progress bar
                    for object_id, question, model_output, ground_truth, gpt_score, reason, prompt_tokens, completion_tokens in pool.imap_unordered(self.evaluate_result, results):
                        self.total_scores += gpt_score if gpt_score != -1 else 0
                        self.total_predictions += 1
                        self.prompt_tokens += prompt_tokens
                        self.completion_tokens += completion_tokens
...
def start_evaluation(results, output_dir, output_file, eval_type="open-free-form-classification", model_type="gpt-3.5-turbo-0613",
                        parallel=True, num_workers=20, task_type="General"):
    """
    Args:
    ...
    if eval_type == "open-free-form-classification":
        ...
        if isinstance(results, dict):
            results["task_type"] = task_type   # ✅ 注入 task_type
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task_type", type=str, default="Captioning",
                    help="The task type name to inject into the prompt (e.g., Knowledge, Reasoning).")
    ...
    start_evaluation(results=args.results_path, output_dir=args.output_dir, output_file=output_file, eval_type=args.eval_type, model_type=args.model_type, 
                        parallel=args.parallel, num_workers=args.num_workers, task_type=args.task_type)
